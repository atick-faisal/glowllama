...
import (
	...
	"github.com/glowllama/glowllama/progress"
	"github.com/glowllama/glowllama/readline"
	"github.com/glowllama/glowllama/runner"
	"github.com/glowllama/glowllama/server"
	"github.com/glowllama/glowllama/types/model"
	"github.com/glowllama/glowllama/types/syncmap"
...
func CreateHandler(cmd *cobra.Command, args []string) error {
	...
	if err := client.Create(cmd.Context(), req, fn); err != nil {
		if strings.Contains(err.Error(), "path or Modelfile are required") {
			return fmt.Errorf("the glowllama server must be updated to use `ollama create` with this client")
		}
		return err
	}
...
func checkServerHeartbeat(cmd *cobra.Command, _ []string) error {
	...
	if err := client.Heartbeat(cmd.Context()); err != nil {
		...
		if err := startApp(cmd.Context(), client); err != nil {
			return fmt.Errorf("glowllama server not responding - %w", err)
		}
	}
	return nil
}
...
func versionHandler(cmd *cobra.Command, _ []string) {
	...
	if err != nil {
		fmt.Println("Warning: could not connect to a running Glowllama instance")
	}

	if serverVersion != "" {
		fmt.Printf("glowllama version is %s\n", serverVersion)
	}
